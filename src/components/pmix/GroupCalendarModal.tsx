 ...
 * GroupCalendarModal
 *
 * Generic calendar popup for a "group" of menu items (beverages, curries,
 * future categories). Shows:
 *   - Monthly heat-mapped calendar of the group's total daily orders
 *   - Per-item breakdown table with expandable daily detail
 *   - Excel export (two sheets: Daily Total + By Item)
 *
 * Reusable by passing a `fetchFn` that returns { days, byItem } and a
 * `color` theme.
 */
...
// ─── Shared types ─────────────────────────────────────────────────────────────
export interface GroupDailyItem {
    itemName:  string;
    totalQty:  number;
    avgPerDay: number;
    days:      { date: string; qty: number }[];
}
export interface GroupDailyResult {
    days:   { date: string; qty: number }[];
    byItem: GroupDailyItem[];
}
...
// ─── Colour themes ────────────────────────────────────────────────────────────
export type GroupColor = "purple" | "amber";

const COLOR: Record<GroupColor, {
    dot:       string;
    heat:      string[];
    val:       string;
    totalText: string;
    ...
    purple: {
        ...
        totalText: "text-purple-600 dark:text-purple-300",
        rowHover:  "hover:bg-purple-50 dark:hover:bg-purple-950/30",
        rowActive: "active:bg-purple-100 dark:active:bg-purple-900/40",
        bar:       "bg-purple-500",
    ...
    amber: {
        ...
        totalText: "text-amber-600 dark:text-amber-300",
        rowHover:  "hover:bg-amber-50 dark:hover:bg-amber-950/30",
        rowActive: "active:bg-amber-100 dark:active:bg-amber-900/40",
        bar:       "bg-amber-500",
...
function heatBg(val: number, max: number, palette: string[]) {
    if (!max || !val) return "";
    const t = val / max;
    if (t < 0.25) return palette[0];
    if (t < 0.5)  return palette[1];
    if (t < 0.75) return palette[2];
    return palette[3];
}
...
interface Props {
    ...
    open:      boolean;
    onClose:   () => void;
    /** Async fetcher; receives (group, from, to) and returns days + byItem. */
    fetchFn:   (group: string, from: string, to: string) => Promise<GroupDailyResult>;
    /** Visual theme. */
    color?:    GroupColor;
    /** Optional file-name prefix for the Excel export. */
    exportPrefix?: string;
}

// ─── Component ────────────────────────────────────────────────────────────────
export default function GroupCalendarModal({
    group, rangeFrom, rangeTo, open, onClose,
    fetchFn, color = "purple", exportPrefix,
}: Props) {
    const c = COLOR[color];

    const [loading,       setLoading]       = useState(false);
    const [dayMap,        setDayMap]        = useState<Map<string, number>>(new Map());
    const [byItem,        setByItem]        = useState<GroupDailyItem[]>([]);
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await fetchFn(group, rangeFrom, rangeTo);
    ...
    }, [open, group, rangeFrom, rangeTo, fetchFn]);
    ...
    function handleExport() {
        ...
        const prefix    = exportPrefix ? `${exportPrefix}_` : "";
        XLSX.writeFile(wb, `${prefix}${safeGroup}_${rangeFrom}_to_${rangeTo}.xlsx`);
    }
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-7 gap-0.5 sm:gap-1">
                                {cells.map((day, idx) => {
                                    ...
                                    const heat   = qty ? heatBg(qty, monthMax, c.heat) : "";
                                    return (
                                        ...
                                        `}>
                                            ...
                                            {qty != null ? (
                                                <div className="mt-0.5 px-0.5 w-full">
                                                    <div className={`text-[10px] sm:text-[11px] font-bold tabular-nums leading-tight ${c.val}`}>{qty}</div>
                            ...
                            {daysWithData > 0 ? (
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-2 border-t border-border text-xs text-muted-foreground">
                                    ...
                                    <span>Range total <strong className={c.totalText}>{monthTotal.toLocaleString()} orders</strong></span>
                                    <div className="flex items-center gap-1 ml-auto">
                                        <span className="text-[10px]">Low</span>
                                        {c.heat.map(cl => <div key={cl} className={`w-2.5 h-2.5 rounded-sm border border-border/40 ${cl}`} />)}
                            ...
                            {byItem.length > 0 && (
                                <div className="rounded-xl border border-border overflow-hidden">
                                            ...
                                            return (
                                                <div key={it.itemName}>
                                                    <button
                                                        ...
                                                        className={`w-full grid grid-cols-[1fr_auto_auto] gap-x-3 px-3 py-2 text-left transition-colors touch-manipulation ${c.rowHover} ${c.rowActive}`}
                                                    >
                                                        <div className="min-w-0">
                                                            <div className="text-xs font-medium text-foreground truncate">{it.itemName}</div>
                                                            <div className="w-full bg-muted rounded-full h-1 mt-1">
                                                                <div className={`${c.bar} h-1 rounded-full`}
                                                                    style={{ width: `${Math.round((it.totalQty / maxItemQty) * 100)}%` }} />
                                                            </div>
                                                        </div>
                                                        <div className={`text-right self-center tabular-nums font-bold ${c.totalText} text-xs shrink-0`}>
                                                            ...
                                                            <div className="grid grid-cols-[auto_1fr_auto] gap-x-3 max-h-48 overflow-y-auto text-xs">
                                                                {(() => {
                                                                    ...
                                                                    for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
                                                                        const iso = d.toISOString().slice(0, 10);
                                                                        rows.push({ date: iso, qty: itemDayMap.get(iso) ?? 0 });
                                                                    }
                                                                    ...
                                                                    return rows.map(({ date, qty }) => (
                                                                        <div key={date} className="contents">
                                                                            ...
                                                                            <div className="self-center">
                                                                                <div className={`h-1.5 ${c.barBg} rounded-full overflow-hidden`}>
                                                                                    <div className={`h-full ${c.bar} rounded-full`}
                                    ...
                                    <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 px-3 py-2 border-t border-border bg-muted/20 text-xs font-bold">
                                        <div>TOTAL</div>
                                        <div className={`text-right ${c.totalText} tabular-nums`}>
