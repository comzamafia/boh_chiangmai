 ...
 *
 * Shared aggregation for the Server Performance dashboard. Used by BOTH:
 *   - the in-app admin route  (/api/server-perf/dashboard)
 *   - the public report API   (/api/public/server-performance)
 * so the numbers can never drift between the UI and external consumers.
 *
 * Aggregates server-sales rows per staff member over a date range and computes
 * multi-dimensional KPIs plus a weighted, peer-normalised Performance Score.
 */
...
export const SCORE_WEIGHTS = {
    salesPerHour: 0.35, avgPerGuest: 0.25, drinkPct: 0.20, dessertPer100: 0.12, discount: 0.08,
};
...
export interface ServerPerfRow {
    ...
    drinkSales: number;
    foodPct: number; beveragePct: number; alcoholPct: number; dessertPct: number; drinkPct: number;
    dessertPer100: number; liquorPerGuest: number;
    score: number;
}
...
export interface ServerPerfResult {
    ...
    coverage: { date: string; serverCount: number; uploadedAt: Date }[];
}

/**
 * Build the full Server Performance payload for [fromStr, toStr] (YYYY-MM-DD).
 * Dates are interpreted in UTC to match how business dates are stored.
 */
export async function buildServerPerformance(fromStr: string, toStr: string): Promise<ServerPerfResult> {
    ...
    const scored: ServerPerfRow[] = base.map(s => {
    ...
    return { range: { from: fromStr, to: toStr }, servers: scored, team, weights: W, coverage };
}
