...
// ─── KPI card colour tokens (tailwind utility strings) ───────────────────────
const KPI = {
    food:     {
        border:  "border-rose-300/70 dark:border-rose-700",
        bg:      "bg-rose-50/40 dark:bg-rose-950/20",
        label:   "text-rose-600 dark:text-rose-400",
        big:     "text-rose-600 dark:text-rose-400",
    ...
    beverage: {
        border:  "border-blue-300/70 dark:border-blue-700",
        bg:      "bg-blue-50/40 dark:bg-blue-950/20",
        label:   "text-blue-600 dark:text-blue-400",
        big:     "text-blue-600 dark:text-blue-400",
        icon:    GlassWater,
    },
    dessert:  {
        border:  "border-orange-300/70 dark:border-orange-700",
        bg:      "bg-orange-50/40 dark:bg-orange-950/20",
        label:   "text-orange-600 dark:text-orange-400",
        big:     "text-orange-600 dark:text-orange-400",
        icon:    Cake,
    },
};
...
export default function PmixDashboardPage() {
    ...
    // Load dashboard data when upload changes
    const load = useCallback(async () => {
        if (!selectedId) return;
        setLoading(true);
        setError(null);
        try {
            const res = await pmixApi.dashboard(selectedId);
            setData(res);
        } catch (e) {
    ...
    const dateLabel = data ? formatDateMonDay(data.businessDate) : "";

    return (
        <div className="space-y-5 p-4 sm:p-6 max-w-6xl mx-auto">

            {/* ── Top bar: upload picker + actions ───────────────────────── */}
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Daily product mix snapshot · click Export PDF for a printable copy
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button variant="outline" size="icon" onClick={load} disabled={loading || !selectedId}
                        title="Refresh" className="h-9 w-9">
            ...
            {!loading && !data && !error && uploads.length === 0 && (
    ...
    return (
        <Card className={`border-2 ${theme.border} ${theme.bg}`}>
            <CardContent className="px-4 py-3">
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">of Total Sales</p>
            </CardContent>
        </Card>
    );
}
